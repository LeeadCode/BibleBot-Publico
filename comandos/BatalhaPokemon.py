...
from entidades.pokemonBot import PokemonBot
...
class Pokemon(commands.Cog):
    ...
    @commands.command('duelo')
    async def inicia_comando(self, ctx: commands.Context, oponente: discord.User):
        secao = asyncio.create_task(self.duelo(ctx, oponente))
        await secao
    ...
    async def get_pokemon(self, ctx):
        ...
        pokemon = await controller_pokemon.get_pokemon(nome_pokemon=pokemon_aleatorio[0])
        await msg_carregando.delete()

        embed = discord.Embed(title=pokemon_aleatorio[0], color=0xFF0000)
        embed.set_author(name="Você encontrou um: ")
        embed.set_thumbnail(url=pokemon.arte_oficial)
        embed.add_field(name="**1-Capturar**", value="**2-Fugir**")

        await ctx.send(embed=embed)

        msg = await self.bot.wait_for('message', check=self.check_caca(ctx.author))

        if(msg.content == '2'):
            await ctx.send('Você corre como um covarde!')
            await msg.delete()
            return

        await msg.delete()
        oponente = PokemonBot(pokemon.nome)

        secao = asyncio.create_task(self.duelo(ctx, oponente, pokemon))

        if(await secao):
            if(ControllerPokemon.calcula_chance_captura()):
                await ctx.send(f'{pokemon.nome} Capturado!')
            else:
                await ctx.send(f'{pokemon.nome} Escapou!')
        else:
            await ctx.send(f'Seu pokemon Desmaiou!')
    ...
    # Check
    def check_caca(self, author):
        '''Checa se o usuário que responder é o usuário desafiado'''
        def inner_check(message):
            return message.author == author and message.content in ['1', '2']
        return inner_check
    ...
    async def prepara_duelo(self, ctx: commands.Context, oponente: discord.User, pokemon=None):
        escolhas = []

        if(not await self.escolha(ctx.author, ctx, escolhas)):
            exit

        if(not oponente.id == 0):
        ...
        if(pokemon != None and oponente.id == 0):
            pokemon_adversario = pokemon
        else:
    ...
    async def duelo(self, ctx: commands.Context, oponente: discord.User, pokemon=None):
        pokemon_desafio, pokemon_adversario = await self.prepara_duelo(ctx, oponente, pokemon)
